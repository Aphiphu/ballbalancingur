...
def calc_angle(pos, i):
    global integral, derivative, time_step, xs, ys,t0, t, e 
...
if __name__ == '__main__':
    ...
    while (cap.isOpened()):
        ...
        if len(contours) > 0:
            ...
            cv2.circle(frame,center,radius=int((pixel_off_x+pixel_off_y)/2), color=(0, 0, 255), thickness=-1)
            print(f"x:{x},y:{y}")            
        ...
        cv2.imshow("Frame", frame)
            #if (abs(xs[-1]-set_x)<pixel_off_x) and (abs(ys[-1]-set_y)<pixel_off_y):
        if cv2.waitKey(1) & 0xFF == ord('q'):
            ...
            # df['time']=time
            # df.to_csv("C:/Users/User3/Desktop/ball_plate.csv",index=False)
            break
    cap.release()
    cv2.destroyAllWindows()
    data = {"x":xs,"y":ys,"angle_x":angle_stored_x,"angle_y":angle_stored_y,"Time":t}
    time_data = {"Time":t}
    df = pd.DataFrame(data)
    df.to_csv("C:/Users/User3/Desktop/kc_1_corner1.csv",index=False)
    #print(df.head())
    plot_set_x = np.ones(len(t)) *set_x
    plot_set_y = np.ones(len(t)) *set_y
    df2 = pd.DataFrame(time_data)
    print(df2.head(n=20))
    plt.subplot(2,1,1)
    plt.plot(t,xs)
    plt.plot(t,plot_set_x)
    plt.subplot(2,1,2)
    plt.plot(t,ys)
    plt.plot(t,plot_set_y)
    plt.legend()
    plt.show()
